...
public class Graph {
    ...
    private List<Node> allPrereqs = new ArrayList<Node>();
    ...
    public Set<String> getAllCourseNames(){
        ...
        for (Map.Entry q: graph.entrySet()){
            courseNames.add(q.getKey().toString());
        }
        //System.out.println(courseNames.toString()); //tested
        return courseNames;
    }
    ...
    public List<Node> computeAllPrereqs(String name) {
        ...
        allPrereqs = new ArrayList<Node>();
        if (!graph.isEmpty())
            return cAPHelper( name );
        return null;
    }
     ...
     */
    private List<Node> cAPHelper(String name){//does work but doesn't give errors
        for (Node prereq : graph.get(name).getNeighbors()) {
            if (!visited.contains(prereq)) {//if we haven't visited this node yet
                visited.add(prereq);
                //if is a prereq: don't know how to determine if a prereq.
                allPrereqs.add(prereq);
                cAPHelper(prereq.getName());
            }
        }
        return allPrereqs;
    }
